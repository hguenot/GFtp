...
class GFtpParser
{
	...
	function __construct()
	{
		...
		//parser
		$this->symbols_ = 		json_decode('{"error":2,"expressions":3,"PROTOCOL":4,"e":5,"EOF":6,"host":7,"user":8,"WORD":9,":":10,"PORT":11,"@":12,"$accept":0,"$end":1}', true);
		$this->terminals_ = 	json_decode('{"2":"error","4":"PROTOCOL","6":"EOF","9":"WORD","10":":","11":"PORT","12":"@"}', true);
		$this->productions_ = 	json_decode('[0,[3,3],[5,1],[5,2],[7,1],[7,3],[8,2],[8,4]]', true);
		$this->table = 			json_decode('[{"3":1,"4":[1,2]},{"1":[3]},{"5":3,"7":4,"8":5,"9":[1,6]},{"6":[1,7]},{"6":[2,2]},{"7":8,"9":[1,9]},{"6":[2,4],"10":[1,10],"12":[1,11]},{"1":[2,1]},{"6":[2,3]},{"6":[2,4],"10":[1,12]},{"9":[1,14],"11":[1,13]},{"9":[2,6]},{"11":[1,13]},{"6":[2,5]},{"12":[1,15]},{"9":[2,7]}]', true);
		$this->defaultActions = json_decode('{"4":[2,2],"7":[2,1],"8":[2,3],"11":[2,6],"13":[2,5],"15":[2,7]}', true);
		
		//lexer
		$this->rules = 			array("/^(?:\\s+)/","/^(?:ftp:\\/\\/)/","/^(?:[0-9]+\\b)/","/^(?:[a-zA-Z0-9-_.]+)/","/^(?::)/","/^(?:@)/","/^(?:$)/","/^(?:.)/");
		$this->conditions = 	json_decode('{"INITIAL":{"rules":[0,1,2,3,4,5,6,7],"inclusive":true}}', true);
...
break;
case 3: 
		$thisS = array_merge($S[$O-1],$S[$O]);
      
break;
case 4: 
		$thisS = array('host' => $S[$O]);
      
break;
case 5: 
		$thisS = array('host' => $S[$O-2], 'port' => $S[$O]);
      
break;
case 6: 
		$thisS = array('user' => $S[$O-1], 'pass' => '');
      
break;
case 7: 
		$thisS = array('user' => $S[$O-3], 'pass' => $S[$O-1]);
